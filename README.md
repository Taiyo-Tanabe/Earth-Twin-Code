...
世界で起きる紛争やクーデターは、政治的な意思決定だけから生まれるのではない。経済の悪化、ガバナンスの崩壊、隣国の不安定化、気候変動、食料危機、難民の流出——無数の要因が複雑に絡み合い、あるとき臨界点を超えることで事態は動く。

Earth Twin は、そのすべての要因を同時に観測し、世界の現在の状態をできる限り忠実に再現しようとするシステムだ。地震活動・気象・疾病・食料価格・民主主義の侵食・武力紛争など13種以上のリアルタイムデータを継続的に収集し、過去数十年の歴史データで学習した機械学習モデルが各国のリスクを算出する。さらにAIエージェントが6時間ごとに新たなオープンデータソースを自律的に発見・統合し、モデルの再現精度を高め続ける。

出力は確率だ。世界の再現は常に不完全であり、観測できない要因は無数にある。0から1の数値は、既知の要因から導かれる現在の状態の推定値であり、絶対的な予言ではない。
...
ダークモードのAIダッシュボード。グリッドテクスチャの背景、スキャンラインアニメーション、ヴィネット効果を持つインタラクティブな世界地図を中心に、3種のリスクレイヤー切り替え、**予測年セレクタ（2026/2027）**、国別リスク詳細パネル、リアルタイムTOP5リストを備える。
...
モデルはカレンダー年単位で予測する。現在は **2026年** と **2027年** の2年分のモデルを保有し、UIで切り替えて比較できる。

```
horizon = 現在年 − データ最新年 + n  (n=1 → 来年, n=2 → 再来年)
例: データ年 2024、今 2026:
    shift(-2) → 2026年モデル
    shift(-3) → 2027年モデル
```

毎年2月（UCDP更新）と4月（V-Dem更新）の自動再学習で新年のモデルが追加される。
